...
def save(headless=True):
    ...
    for episode in range(episodes):
        x_vel_cmd = torch.tensor([random.uniform(-1.5, 1.5) for _ in range(env.num_envs)])
        y_vel_cmd = torch.tensor([random.uniform(-1.5, 1.5) for _ in range(env.num_envs)])
        yaw_vel_cmd = torch.tensor([random.uniform(-0.5, 0.5) for _ in range(env.num_envs)])
        ...
        recorded_obs = torch.stack(recorded_obs, dim=1) # 250*(500,42) -> (250*500,42)
